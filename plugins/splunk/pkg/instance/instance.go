...
// Config is the structure of the configuration for a single Splunk instance.
type Config struct {
...
// Instance represents a single Splunk instance, which can be added via the configuration file.
...
func (i *Instance) GetLogs(ctx context.Context, query string, timeStart, timeEnd int64) (*Data, error) {
	...
	log.WithFields(logrus.Fields{"query": string(body)}).Debugf("Run Splunk query")
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		...
		log.WithFields(logrus.Fields{"took": data.Took, "hits": data.Hits, "documents": len(data.Documents), "buckets": len(data.Buckets)}).Debugf("Splunk query results")
...
// New returns a new Splunk instance for the given configuration.
