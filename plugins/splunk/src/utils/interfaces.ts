...
// IOptions is the interface for all options, which can be set for an Splunk query.
...
// IPanelOptions are the options for the panel component of the Splunk plugin.
...
// ILogsData is the interface of the data returned from our Go API for the Splunk plugin. The interface must
...
// IBucket is the interface for a single bucket returned by the Splunk API. For that we are using the naming
// convention from Splunk.
